...
namespace GuiltineSin.Zone.World.Actors.Characters
{
	public partial class Character
	{
		...
		private static readonly (int Level, int Slot)[] GuiltineSinLevelingJobUnlocks =
		[
			(45, 0),
			(85, 1),
			(120, 2),
		...
		private static readonly Dictionary<JobClass, JobId[]> GuiltineSinDefaultAdvancedJobs = new()
		{
		...
		public int EnsureGuiltineSinLevelingJobProgression(bool updateClient = true)
		{
			if (this.JobId == JobId.None || this.JobClass == JobClass.GM)
				return 0;

			if (!GuiltineSinDefaultAdvancedJobs.TryGetValue(this.JobClass, out var progression))
				return 0;

			var added = 0;
			foreach (var unlock in GuiltineSinLevelingJobUnlocks)
			{
				...
				Log.Info("GuiltineSin leveling: auto-advanced character '{0}' to '{1}' at base level {2} (slot {3}/3).", this.Name, jobId, this.Level, unlock.Slot + 1);
				if (removedSkillStateBuffs > 0)
					Log.Info("GuiltineSin leveling: removed {0} unsafe skill-state buff(s) before auto class advancement for '{1}'.", removedSkillStateBuffs, this.Name);
			}
