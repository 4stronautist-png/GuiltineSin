...
namespace GuiltineSin.Zone.World.Actors.Characters
{
	public partial class Character
	{
		...
		{
			if (monster is Npc npc && (npc.Name.Contains("Marcus") || npc.Name.Contains("Elara") || npc.Name.Contains("Bronius") || npc.Name.Contains("Vesta") || npc.Name.Contains("Henrik") || npc.Name.Contains("Tomas") || npc.Name.Contains("Sigrid")))
				Log.Info($"Quest NPC visible to {this.Name}: {npc.Name} map={this.Map?.ClassName} pos={npc.Position} handle={npc.Handle} genType={npc.GenType} state={npc.State} dialog={npc.DialogName}");
