...
namespace GuiltineSin.Zone.World.Actors.Monsters
{
	...
	{
		private const int GuiltineSinQuestBalancedVubbeFighterHp = 1000;
		...
		private void InitProperties()
		{
			...
			this.ApplyGuiltineSinQuestBalancedVubbeFighterHpOverride();
		...
		public virtual void Kill(ICombatEntity killer)
		{
			...
			var suppressDeathSkillCancel = this.CheckBoolTempVar("GuiltineSin.SuppressDeathSkillCancel");
		...
		private void ApplyGuiltineSinQuestBalancedVubbeFighterHpOverride()
		{
			if (!IsGuiltineSinQuestBalancedVubbeFighterType(this.Data?.ClassName))
				return;

			this.Properties.Overrides.SetFloat(PropertyName.MHP, GuiltineSinQuestBalancedVubbeFighterHp);
		}

		private static bool IsGuiltineSinQuestBalancedVubbeFighterType(string className)
		{
			var normalized = NormalizeGuiltineSinMonsterTypeName(className);
			return normalized.Contains("goblinwarrior", StringComparison.OrdinalIgnoreCase);
		}

		private static string NormalizeGuiltineSinMonsterTypeName(string className)
		{
			if (string.IsNullOrWhiteSpace(className))
				return string.Empty;
