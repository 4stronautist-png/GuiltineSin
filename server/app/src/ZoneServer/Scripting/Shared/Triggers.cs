...
namespace GuiltineSin.Zone.Scripting.Shared
{
	public static class Triggers
	{
		...
		{
			if (args.Initiator is not Character character || args.Trigger is not Npc npc)
				return;

			var hookOwner = args.Type switch
			{
				TriggerType.Enter => npc.EnterName,
				TriggerType.Leave => npc.LeaveName,
				_ => null,
			};

			if (string.IsNullOrWhiteSpace(hookOwner))
				return;

			Dialog dialog;
			try
			{
				dialog = new Dialog(character, npc);
			}
		...
		{
			await COMMON_QUEST_HANDLER(args);
		}
