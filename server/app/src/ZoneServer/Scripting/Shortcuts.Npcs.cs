...
namespace GuiltineSin.Zone.Scripting
{
	public static partial class Shortcuts
	{
		...
		private static int CreateDynamicNpcGenType(int monsterId, string mapClassName, double x, double z)
		{
			unchecked
			{
				...
				var key = $"{monsterId}:{mapClassName}:{x:F3}:{z:F3}";
				var hash = fnvOffset;
				...
				// Keep dynamically scripted NPCs in the legacy visible/stateful
				// genType range the client expects for regular map NPCs.
				return 1000 + (int)(hash % 900_000);
			}
		}
		...
		public static Npc AddNpc(int monsterId, string name, string uniqueName, string map, double x, double z, double direction, DialogFunc dialog = null)
		{
			...
			// Keep the dynamic/lambda path aligned with the generated AddNpc
			// overloads, because quest NPC scripts frequently pass localization
			// keys and tagged display names through this path.
			if (Dialog.IsLocalizationKey(name))
			{
				name = Dialog.WrapLocalizationKey(name);
			}
			...
			var genType = CreateDynamicNpcGenType(monsterId, mapObj.Data.ClassName, x, z);
			var location = new Location(mapObj.Id, pos);
			var dir = new Direction(direction);

			var npc = new Npc(monsterId, name, location, dir, genType);
			npc.UniqueName = uniqueName;

			if (dialog != null)
				npc.SetClickTrigger("DYNAMIC_DIALOG", dialog);

			ZoneServer.Instance.World.NPCs.TryAdd(uniqueName, npc);

			if (name.Contains("Marcus") || name.Contains("Elara") || name.Contains("Bronius") || name.Contains("Vesta") || name.Contains("Henrik") || name.Contains("Tomas") || name.Contains("Sigrid"))
				Log.Info($"Quest NPC spawn (dynamic): {name} on {mapObj.ClassName} at {pos} dialog={npc.DialogName} genType={npc.GenType}");
