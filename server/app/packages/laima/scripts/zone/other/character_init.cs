...
public class CharacterInitializationScript : GeneralScript
{
	...
	public void OnPlayerLoggedIn(object sender, PlayerEventArgs args)
	{
		...
		var vars = character.Variables;
		var classicStartInitialized = vars.Perm.GetBool(ClassicStartInitVar, false);

		if (!classicStartInitialized)
		{
			EnsureClassicStart(character);
			InitCharacter(character);
			vars.Perm.SetBool(ClassicStartInitVar, true);
		}
		else if (character.Level <= 1 && (character.MapId == 4250 || character.MapId == 1001))
		{
			// Recover characters that were already saved inside the
			// unintended tutorial instance or Klaipeda while the local
			// fork was still being stabilized.
			EnsureClassicStart(character);
		}
	...
	private static void EnsureClassicStart(Character character)
	{
		...
		character.MapId = ClassicStartMapId;
		character.Position = ClassicStartPosition;
		character.Direction = new Direction(0);

		character.Etc.Properties.SetFloat(PropertyName.FirstPlay, 0);
		character.SessionObjects.Main.Properties.SetString(PropertyName.QSTARTZONETYPE, "StartLine1");
	}
