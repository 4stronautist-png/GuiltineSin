...
function STATUS_INFO()
    ...
    if returnY ~= y then y = returnY + 3; end
    returnY = STATUS_POISON_MASTERY_VALUE(pc, opc, frame, gboxctrl, y);
    if returnY ~= y then y = returnY + 3; end
    returnY = STATUS_SUMMON_CONTROL_VALUE(pc, opc, frame, gboxctrl, y);
    if returnY ~= y then y = returnY + 3; end
...
function STATUS_HAS_POISON_MASTERY_JOB()
    local poisonMasteryJobs = {
        [3006] = true, -- Wugushi
        [4014] = true, -- Plague Doctor
        -- Add future Poison Mastery classes here.
    };
    ...
    for i = 0, jobCount - 1 do
        ...
        if jobInfo ~= nil and poisonMasteryJobs[jobInfo.jobID] == true then
            return true;
        end
    end
...
function STATUS_POISON_MASTERY_VALUE(pc, opc, frame, gboxctrl, y)
    if pc == nil then
        return y;
    end

    if STATUS_HAS_POISON_MASTERY_JOB() == false then
        gboxctrl:RemoveChild('PoisonMastery');
        return y;
    end

    local intValue = TryGetProp(pc, 'INT', 0);
    local percent = math.floor(math.min(100, math.max(0, intValue / 1900 * 100)) + 0.5);
    local controlSet = gboxctrl:CreateOrGetControlSet('status_stat', 'PoisonMastery', 0, y);
    tolua.cast(controlSet, "ui::CControlSet");
    ...
    title:SetUseOrifaceRect(true);
    stat:SetUseOrifaceRect(true);
    title:SetText('{#006633}Poison Mastery{/}');
    title:SetTextTooltip("Scales with the user's INT.");
    stat:SetText('{#006633}' .. percent .. '%{/}');
    stat:SetTextTooltip("Scales with the user's INT.");

    controlSet:Resize(controlSet:GetWidth(), stat:GetHeight());
    return y + controlSet:GetHeight();
end

function STATUS_HAS_SUMMON_CONTROL_JOB()
    local summonControlJobs = {
        [2006] = true, -- Sorcerer
        [2009] = true, -- Necromancer
        [2022] = true, -- Bokor
        -- Add future Summon Control classes here.
    };
    ...
    for i = 0, jobCount - 1 do
        local jobInfo = pcJobInfo:GetJobInfoByIndex(i);
        if jobInfo ~= nil and summonControlJobs[jobInfo.jobID] == true then
            return true;
        end
    end
...
function STATUS_SUMMON_CONTROL_STAT_RATE(value)
    local normalized = math.max(0, math.min(1, value / 5000));
    return math.pow(normalized, 0.85);
end

function STATUS_SUMMON_CONTROL_PERCENT(pc)
    if pc == nil then
        return 0;
    end

    local intValue = TryGetProp(pc, 'INT', 0);
    local conValue = TryGetProp(pc, 'CON', 0);
    local sprValue = TryGetProp(pc, 'MNA', 0);
    local percent = (STATUS_SUMMON_CONTROL_STAT_RATE(intValue) + STATUS_SUMMON_CONTROL_STAT_RATE(conValue) + STATUS_SUMMON_CONTROL_STAT_RATE(sprValue)) / 3 * 100;
    return math.floor(math.min(100, math.max(0, percent)) + 0.5);
end

function STATUS_SUMMON_CONTROL_VALUE(pc, opc, frame, gboxctrl, y)
    if pc == nil then
        return y;
    end

    if STATUS_HAS_SUMMON_CONTROL_JOB() == false then
        gboxctrl:RemoveChild('SummonControl');
        return y;
    end

    local percent = STATUS_SUMMON_CONTROL_PERCENT(pc);
    local controlSet = gboxctrl:CreateOrGetControlSet('status_stat', 'SummonControl', 0, y);
    tolua.cast(controlSet, "ui::CControlSet");
    ...
    title:SetUseOrifaceRect(true);
    stat:SetUseOrifaceRect(true);
    title:SetText('{#3399FF}Summon Control{/}');
    title:SetTextTooltip("Scales from INT, CON and SPR.");
    stat:SetText('{#3399FF}' .. percent .. '%{/}');
    stat:SetTextTooltip("Each 1% Summon Control increases summon damage by 1%.");
