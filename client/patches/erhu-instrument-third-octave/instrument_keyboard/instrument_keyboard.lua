...
function GET_INSTRUMENT_KEYBOARD_CNT_BY_TYPE(type)
	if type == "ep13cherryblossom" then
		return 25;
	end

	return GET_INSTRUMENT_KEYBOARD_CNT();
end
...
function INSTRUMENT_KEYBOARD_OPEN(frame, msg, type)
	...
	local maxCnt = GET_INSTRUMENT_KEYBOARD_CNT();
	local cnt = GET_INSTRUMENT_KEYBOARD_CNT_BY_TYPE(type);
	for i = 0, maxCnt - 1 do
		local slot = GET_CHILD(frame, "slot"..i+1);
		local slotString = 'QuickSlotExecute'..(i+1);
